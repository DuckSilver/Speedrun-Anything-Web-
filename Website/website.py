...
def main(page: ft.Page):
    ...
    def show_login_page():
        ...
        join_btn = ft.ElevatedButton("Enter Room", bgcolor="blue", color="white", width=200, height=50, on_click=attempt_login)
    ...
    def show_main_page(is_sync=False):
        ...
        def open_import_dialog(e):
            ...
            import_input = ft.TextField(label="Paste copied list data here...", multiline=True, min_lines=5, max_lines=8)
        ...
        def handle_reorder(list_name, action):
            ...
            if action == "pin":
                ...
                if is_pinned:
                    pinned.remove(list_name); pinned.insert(0, list_name)
                else:
                    unpinned.remove(list_name); unpinned.insert(0, list_name)
            elif action == "bottom":
                if is_pinned:
                    pinned.remove(list_name); pinned.append(list_name)
                else:
                    unpinned.remove(list_name); unpinned.append(list_name)
        ...
        def create_list_row(list_name):
            ...
            def rename_list(e=None):
                ...
                dialog = ft.AlertDialog(
                    ...
                    actions=[ft.TextButton("Cancel", on_click=cancel_rename), ft.TextButton("Save", on_click=save_rename)]
                )
                page.overlay.append(dialog)
                dialog.open = True
                page.update()
            ...
            def open_options_menu(e):
                def execute_action(action):
                    ...
                    page.update()
                    if action == "rename": rename_list()
                    elif action == "delete": delete_list()
                    elif action == "export":
                        json_str = json.dumps(room_lists[list_name])
                        export_input = ft.TextField(value=json_str, multiline=True, min_lines=5, max_lines=8, read_only=True, label="Raw Data")
                        ...
                        export_dialog = ft.AlertDialog(
                            ...
                            content=ft.Column([
                                ft.Text("If the Auto-Copy button fails, manually highlight and copy (Ctrl+C) the text below:", size=12, color="grey400"),
                                export_input
                            ], tight=True),
                            actions=[
                                ft.TextButton("Close", on_click=lambda _: [setattr(export_dialog, 'open', False), page.update()]),
                        ...
                        page.update()
                    else: handle_reorder(list_name, action)

                menu_dialog = ft.AlertDialog(
                    title=ft.Text(f"Options", size=20, weight=ft.FontWeight.BOLD),
                    content=ft.Column([
                        ft.TextButton("📌 Pin/Unpin List", on_click=lambda _: execute_action("unpin" if is_pinned else "pin")),
                    ...
                    actions=[ft.TextButton("Cancel", on_click=lambda _: [setattr(menu_dialog, 'open', False), page.update()])]
                )
                page.overlay.append(menu_dialog)
                menu_dialog.open = True
                page.update()

            edit_btn = ft.Container(content=ft.Text("⋮", size=24, weight=ft.FontWeight.BOLD), tooltip="Options", on_click=open_options_menu, padding=10)
            ...
            subtitle_col = ft.Column([ft.Text(f"{int(list_progress * 100)}% Complete", size=12), mini_prog_bar, best_text], spacing=2)
        ...
        page.add(ft.SafeArea(content=ft.Column([header, ft.Row([new_list_input, add_btn, import_btn]), ft.Divider(), lists_view], expand=True), expand=True))
        page.update()
    ...
    def show_list_mode(list_name, is_sync=False):
        ...
        def create_category_tile(cat_name, cat_tasks):
            ...
            def edit_task(t):
                ...
                type_dropdown = ft.Dropdown(
                    ...
                    options=[ft.dropdown.Option("duration", "Timer/Duration"), ft.dropdown.Option("clock", "Time of Day")]
                )
                ...
                if t.get("time"):
                    if t.get("time_type") == "duration":
                        parts = t["time"].split(":")
                        if len(parts) == 3: th, tm, ts = parts[0], parts[1], parts[2]
                        elif len(parts) == 2: tm, ts = parts[0], parts[1]
                        elif len(parts) == 1: ts = parts[0]
                ...
                timer_row = ft.Row([timer_h, ft.Text(":"), timer_m, ft.Text(":"), timer_s], visible=(t.get("time_type", "duration") == "duration"))

                clock_h = ft.TextField(label="H", width=70, value=ch)
                clock_m = ft.TextField(label="M", width=70, value=cm)
                clock_ampm = ft.Dropdown(options=[ft.dropdown.Option("AM"), ft.dropdown.Option("PM")], width=80, value=ampm)
                edit_date_input = ft.TextField(value=str(t.get("date", "")), label="Date (Optional)", expand=True)
                clock_col = ft.Column([ft.Row([clock_h, ft.Text(":"), clock_m, clock_ampm]), edit_date_input], visible=(t.get("time_type") == "clock"))
                ...
                def save_task_edit(e):
                    ...
                    if t["time_type"] == "duration":
                        t["time"] = f"{safe_int(timer_h.value)}:{safe_int(timer_m.value):02d}:{safe_int(timer_s.value):02d}"
                        t["date"] = ""
                    else:
                ...
                dialog = ft.AlertDialog(
                    ...
                    content=ft.Column([edit_task_input, edit_weight_input, type_dropdown, timer_row, clock_col], tight=True),
                    actions=[
            ...
            def refresh_tasks():
                ...
                for task in cat_tasks:
                    ...
                    task_row = ft.Row([cb, ft.TextButton("Edit", on_click=lambda e, t=task: edit_task(t))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
            ...
            def add_task(e):
                ...
                if val:
                    cat_tasks.append({"text": val, "done": False, "time": "", "time_type": "duration", "date": "", "weight": 1.0})
                    page.expanded_categories.add(cat_name) 
                    trigger_global_sync()
            ...
            cat_title_row = ft.Row([cat_title_text, ft.TextButton("Edit", on_click=edit_category)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
        ...
        def open_speedrun_setup(e):
            ...
            radios = ft.RadioGroup(
                content=ft.Column([
                    ...
                    ft.Text("Highlights tasks in red that have not been completed by their set time.", color="grey400", size=12),
        ...
        def open_leaderboard(e):
            ...
            def create_tab_content(mode_key):
                ...
                if not records:
                    return ft.Container(content=ft.Row([ft.Text("No records yet.", color="grey")], alignment=ft.MainAxisAlignment.CENTER), padding=20)
            ...
            def set_tab(mode):
                if mode == "timer":
                    btn_timer.bgcolor = "blue"; btn_clock.bgcolor = "grey800"
                    dynamic_content.content = create_tab_content("timer")
                else:
                    btn_timer.bgcolor = "grey800"; btn_clock.bgcolor = "blue"
        ...
        speedrun_btn = ft.ElevatedButton("Start Speedrun ⏱️", bgcolor="green", color="white", on_click=open_speedrun_setup)

        page.add(ft.SafeArea(content=ft.Column([header, progress_ui, ft.Row([cat_input, add_cat_btn]), ft.Divider(), categories_view, ft.Row([lb_btn, speedrun_btn], alignment=ft.MainAxisAlignment.CENTER)], expand=True), expand=True))
    ...
    def show_speedrun_mode(list_name, categories, mode, show_avg, show_best):
        ...
        def get_pace_text(task_text):
            ...
            if not lb: return ""
            times = [entry["task_times"][task_text] for entry in lb if "task_times" in entry and task_text in entry["task_times"]]
            if not times: return ""
        ...
        clock_container = ft.Column([clock_display], alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        ...
        def mark_completed():
            ...
            entry = {"sort_val": sort_val, "display": final_time_str, "date": today_str, "task_times": current_run_task_times}
            lb = room_lists[list_name]["leaderboards"][mode]
            lb.append(entry)
            ...
            clock_container.controls.clear()
            clock_container.controls.append(ft.Text("Speedrun Complete!", size=24, color="green", weight=ft.FontWeight.BOLD))
        ...
        for cat_name, cat_tasks in categories.items():
                    ...
                    def toggle(e):
                        ...
                        if e.control.value:
                            if mode == "timer":
                                ...
                                now_t = datetime.now()
                                current_run_task_times[task_dict["text"]] = now_t.hour * 3600 + now_t.minute * 60 + now_t.second
                        ...
                        if is_all_done and not speedrun_state["completed"]: mark_completed()
                        elif not is_all_done and speedrun_state["completed"]: resume_run()
                        else: page.update()
            ...
            cat_tile = ft.ExpansionTile(title=ft.Text(cat_name, size=18, weight=ft.FontWeight.BOLD), controls=[tasks_col], expanded=True)
            speedrun_view.controls.append(cat_tile)

        page.add(ft.SafeArea(content=ft.Column([header_btn, ft.Row([clock_container], alignment=ft.MainAxisAlignment.CENTER), progress_ui, ft.Divider(), speedrun_view], expand=True), expand=True))
            ...
            while speedrun_state["active"]:
                ...
                if mode == "timer":
                    ...
                    for task_dict, row_ui, text_ui in active_ui_refs:
                        if not task_dict.get("done") and task_dict.get("time_type") == "duration" and task_dict.get("time"):
...
if __name__ == "__main__":
    ...
    ft.app(target=main, view=ft.AppView.WEB_BROWSER, port=port, host="0.0.0.0", assets_dir=ASSETS_DIR, upload_dir=UPLOAD_DIR)
